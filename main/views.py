...
def individual_details(request, username):
    ...
    n = random.randint(0,22)
    data = requests.get(f"http://127.0.0.1:8000/advocates/{username}")
    return render(request, "main.html" , {"data":data , "n":n})
...
def add_data(request, username):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            response = requests.patch(f"http://127.0.0.1:8000/update-advocate/{username}", json=data)
